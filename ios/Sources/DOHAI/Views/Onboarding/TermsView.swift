//
//  TermsView.swift
//  DOH AI
//
//  Terms and Conditions acceptance screen (first launch)
//

import SwiftUI

struct TermsView: View {
    @Binding var hasAcceptedTerms: Bool
    @State private var hasScrolledToBottom = false
    @State private var scrollOffset: CGFloat = 0

    var body: some View {
        VStack(spacing: 0) {
            // Header
            VStack(spacing: 12) {
                Image(systemName: "doc.text.fill")
                    .font(.system(size: 50))
                    .foregroundStyle(.blue)

                Text("Terms and Conditions")
                    .font(.title.bold())

                Text("Please read and accept to continue")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
            }
            .padding(.vertical, 24)

            Divider()

            // Scrollable Terms Content
            ScrollViewReader { proxy in
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        termsContent

                        // Bottom marker for scroll detection
                        Color.clear
                            .frame(height: 1)
                            .id("bottom")
                            .onAppear {
                                hasScrolledToBottom = true
                            }
                    }
                    .padding()
                }
            }

            Divider()

            // Accept Button
            VStack(spacing: 12) {
                if !hasScrolledToBottom {
                    Text("Please scroll to read the full terms")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }

                Button {
                    withAnimation {
                        hasAcceptedTerms = true
                    }
                } label: {
                    Text("Accept Terms and Conditions")
                        .font(.headline)
                        .frame(maxWidth: .infinity)
                        .padding()
                        .background(hasScrolledToBottom ? Color.blue : Color.gray)
                        .foregroundStyle(.white)
                        .clipShape(RoundedRectangle(cornerRadius: 12))
                }
                .disabled(!hasScrolledToBottom)

                Text("Last updated: January 1, 2026")
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
            }
            .padding()
        }
    }

    private var termsContent: some View {
        VStack(alignment: .leading, spacing: 20) {
            Text("Effective Date: January 1, 2026")
                .font(.subheadline)
                .foregroundStyle(.secondary)

            Group {
                sectionTitle("Welcome to DOH AI!")

                Text("""
                These Terms and Conditions ("Terms") constitute a legally binding agreement between you and DOH AI regarding your use of our application and services. By accessing or using DOH AI, you agree to comply with and be bound by these Terms.
                """)

                sectionTitle("Acceptance of Terms")

                Text("""
                By using DOH AI, you confirm that you are of legal age and capacity to enter into these Terms. If you do not agree to these Terms, you must not use our app.
                """)

                sectionTitle("AI Model Limitations and Disclaimers")

                Text("You acknowledge and agree that:")
                    .fontWeight(.medium)

                bulletPoints([
                    "The AI models in DOH AI may generate content that is incorrect, incomplete, misleading, or inappropriate.",
                    "You should not rely on AI-generated content for medical, legal, financial, or other professional advice.",
                    "The AI models may occasionally produce biased, offensive, or harmful content despite our best efforts to prevent such outputs.",
                    "You are solely responsible for verifying any information or content generated by the AI models before acting upon it.",
                    "DOH AI is not responsible for any decisions, actions, or consequences resulting from your use of AI-generated content.",
                    "The AI models' responses are generated through probabilistic methods and may not reflect current or accurate information.",
                    "Any code, scripts, or technical solutions generated by the AI should be thoroughly reviewed and tested before implementation."
                ])
            }

            Group {
                sectionTitle("Data and Privacy")

                Text("Regarding data privacy and processing:")
                    .fontWeight(.medium)

                bulletPoints([
                    "When using local models, all processing occurs on your device with no data sent to external servers",
                    "When using cloud models, your conversations and prompts are sent to external servers",
                    "You are responsible for the security of your device and any data you input into the app",
                    "You are responsible for ensuring you have the right to use any content you input into the app",
                    "You are responsible for any consequences of sharing AI-generated content with third parties",
                    "You acknowledge that using cloud models involves transmitting your data to third parties"
                ])

                sectionTitle("Third-Party Services")

                Text("Regarding third-party services:")
                    .fontWeight(.medium)

                bulletPoints([
                    "The integration allows you to access third-party AI models through their APIs",
                    "Your use of third-party services is subject to their terms of service and privacy policy",
                    "DOH AI is not responsible for the practices, policies, or content of third-party providers",
                    "You are responsible for reviewing and accepting the terms of any third-party services you access through our app",
                    "DOH AI makes no warranties regarding the availability, reliability, or performance of third-party services",
                    "Your API keys and account management are your responsibility"
                ])
            }

            Group {
                sectionTitle("Prohibited Uses")

                Text("You agree not to use DOH AI:")
                    .fontWeight(.medium)

                bulletPoints([
                    "For any unlawful purpose or to generate content that promotes illegal activities",
                    "To generate content that is discriminatory, hateful, or promotes harm against individuals or groups",
                    "To create misleading or fraudulent content",
                    "To generate spam, malware, or other malicious content",
                    "In any way that could damage, disable, or impair the app's functionality",
                    "To attempt to reverse engineer or extract the AI models",
                    "To generate content that infringes on intellectual property rights",
                    "To attempt to identify or reverse engineer the training data or architecture of the AI models",
                    "For automated or systematic extraction of content from the app"
                ])

                sectionTitle("Disclaimer of Warranties")

                Text("""
                DOH AI IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT ANY WARRANTIES OF ANY KIND, WHETHER EXPRESS OR IMPLIED. We specifically disclaim any implied warranties of merchantability, fitness for a particular purpose, and non-infringement.
                """)
                .fontWeight(.medium)
            }

            Group {
                sectionTitle("Limitation of Liability")

                Text("TO THE MAXIMUM EXTENT PERMITTED BY LAW:")
                    .fontWeight(.bold)

                bulletPoints([
                    "DOH AI SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES",
                    "OUR TOTAL LIABILITY FOR ANY CLAIMS ARISING FROM OR RELATED TO YOUR USE OF THE APP SHALL NOT EXCEED THE AMOUNT YOU PAID FOR THE APP",
                    "WE ARE NOT LIABLE FOR ANY ACTIONS YOU TAKE OR REFRAIN FROM TAKING BASED ON AI-GENERATED CONTENT",
                    "WE ARE NOT LIABLE FOR ANY LOSS OF DATA, PROFIT, GOODWILL, OR BUSINESS OPPORTUNITY",
                    "WE ARE NOT LIABLE FOR ANY INTERRUPTION OF BUSINESS OR ANY OTHER COMMERCIAL DAMAGES OR LOSSES",
                    "WE ARE NOT LIABLE FOR ANY ACTIONS, POLICIES, OR DATA PRACTICES OF THIRD-PARTY SERVICES",
                    "THESE LIMITATIONS APPLY EVEN IF WE HAVE BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES"
                ])

                sectionTitle("Export Control")

                Text("You agree to comply with all applicable export control laws and regulations in your use of the app and its AI capabilities.")

                sectionTitle("Indemnification")

                Text("""
                You agree to indemnify, defend, and hold harmless DOH AI and its officers, directors, employees, and agents from and against any claims, liabilities, damages, losses, and expenses, including reasonable attorneys' fees, arising out of or in any way connected with your use of the app or violation of these Terms.
                """)
            }

            Group {
                sectionTitle("Changes to Terms")

                Text("""
                We reserve the right to modify these Terms at any time. We will notify you of material changes through the app or website. Your continued use after such modifications constitutes acceptance of the updated Terms.
                """)

                sectionTitle("Governing Law and Jurisdiction")

                Text("""
                These Terms are governed by USA law. Any disputes shall be subject to the exclusive jurisdiction of USA courts. You agree to submit to the personal jurisdiction of such courts.
                """)

                sectionTitle("Severability")

                Text("If any provision of these Terms is found to be unenforceable, the remaining provisions will continue in full force and effect.")

                sectionTitle("Contact Us")

                Text("If you have any questions about these Terms, please contact us at:")

                Link("marketing@dealerofhappiness.com", destination: URL(string: "mailto:marketing@dealerofhappiness.com")!)
                    .foregroundStyle(.blue)
            }
        }
        .font(.body)
    }

    private func sectionTitle(_ text: String) -> some View {
        Text(text)
            .font(.headline)
            .padding(.top, 8)
    }

    private func bulletPoints(_ points: [String]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ForEach(points, id: \.self) { point in
                HStack(alignment: .top, spacing: 8) {
                    Text("•")
                    Text(point)
                }
            }
        }
        .padding(.leading, 8)
    }
}

#Preview {
    TermsView(hasAcceptedTerms: .constant(false))
}
